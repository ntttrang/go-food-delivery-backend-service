...
func TestPasswordVerificationDemo(t *testing.T) {
	...
	// Create mock device token repo
	mockDeviceTokenRepo := &mockDeviceTokenRepo{shouldFail: false}

	// Create mock refresh token generator
	mockRefreshTokenGenerator := &mockRefreshTokenGenerator{shouldFail: false}

	// Create authentication handler
	authHandler := NewAuthenticateCommandHandler(mockRepo, mockDeviceTokenRepo, mockTokenIssuer, mockRefreshTokenGenerator)
	...
	t.Run("correct password should authenticate", func(t *testing.T) {
		...
		result, err := authHandler.Execute(context.Background(), req, "Mozilla/5.0 (Test)")
		...
		if result.AccessToken != "mock-jwt-token" {
			t.Errorf("Expected token 'mock-jwt-token', got '%s'", result.AccessToken)
		}

		t.Logf("✅ Password verification successful! Token: %s", result.AccessToken)
	})
